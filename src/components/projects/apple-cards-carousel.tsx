...
export const Card = ({
  ...
  //@ts-expect-error: generic typing of useOutsideClick not provided in local hook
  ...
  return (
    <>
        ...
        {open && (
          <div className="fixed inset-0 z-52 h-screen overflow-auto">
            ...
            >
              ...
              {/* Content with consistent padding and readable colors; remove media */}
              <div className="px-8 pt-8 pb-14 md:px-14">
                <div className="prose max-w-none whitespace-pre-line text-neutral-900">{card.content}</div>
              </div>
            </motion.div>
          </div>
        )}
      ...
      <motion.button
        layoutId={layout ? `card-${card.title}` : undefined}
        onClick={handleOpen}
        className="relative z-10 flex h-80 w-56 flex-col items-start justify-start overflow-hidden rounded-3xl bg-gradient-to-br from-neutral-800 to-neutral-700"
      >
        {/* Card image (if provided in data) */}
        <div className="absolute inset-0 z-20">
          {/* We derive image from Data.tsx's src; when not an image path, background will remain gradient */}
          {card?.category && ((card as unknown as { src?: string }).src ? (
              <Image
                src={(card as unknown as { src?: string }).src as string}
                alt={card.title}
                fill
                sizes="224px"
        ...
        {/* Image removed for text-only cards */}
      </motion.button>
    </>
  );
};
